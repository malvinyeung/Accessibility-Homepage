...
if(d>0&&h.indexOf("globalheaderfooter")==-1){var c=0;
...
(function(){var e="CAMPAIGN",d="campaign",a="experience";
var c=function(h,l){var k="/content/campaigns",n=k+"/"+h,g=n+"/"+l,m=CQ_Analytics.CampaignMgr.getCampaignBy("path",n),j=CQ_Analytics.CampaignMgr.getExperienceBy("path",g);
if(!j){j={title:"(default)",path:CQ_Analytics.CampaignMgr.DEFAULT_EXPERIENCE,id:CQ_Analytics.CampaignMgr.DEFAULT_EXPERIENCE}
}CQ_Analytics.CampaignMgr.setProperties({name:m?m.title:"",path:m?m.path:"",id:m?m.id:"","recipe/name":j?j.title:"","recipe/path":j?j.path:"","recipe/id":j?j.id:""});
if(SfdcWwwBase.config&&SfdcWwwBase.config.isPublish){CQ_Analytics.CampaignMgr.persistence.remove(e)
}};
var f=CQ.shared.HTTP.getParameter(location.href,d),b=CQ.shared.HTTP.getParameter(location.href,a);
CQ_Analytics.CampaignMgr.addListener("initialize",function(){if(SfdcWwwBase.config&&SfdcWwwBase.config.isPublish){c(f,b)
}})
}());